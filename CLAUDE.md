...
A private, single-owner PSX (Pakistan Stock Exchange) portfolio ledger and monthly SIP (systematic investment plan) planner, built with Next.js App Router code executed by **vinext** (a Next.js-compatible runtime built on Vite + React Server Components), deployed as a **Cloudflare Worker** in the owner's own Cloudflare account, with a **D1** (SQLite) database via Drizzle ORM. Auth is Google OAuth sign-in restricted to an email allowlist (`ALLOWED_EMAILS`), not a custom accounts system.
