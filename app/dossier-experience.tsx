...
export default function DossierExperience({draft,company,onChange,onBack,onSave}:Props){
 ...
 const full:DossierCompany={...blank(draft.ticker,company?.name??draft.ticker,''),...details,ticker:draft.ticker,name:details?.name||company?.name||draft.ticker,status:draft.status==='Queue'?'Queued':draft.status,thesis:draft.thesis,risk:draft.risks,catalyst:draft.catalysts,conversation:draft.conversationUrl,week:details?.week??draft.updatedAt,financials:details?.financials??draft.financials.map(f=>({...f,year:Number(f.year),ocf:null,equity:null,dividend:null,source:'',page:'',basis:'',verified:false})),documents:details?.documents??draft.sources.map(url=>({title:url,url,kind:'Report / filing',date:''}))};
