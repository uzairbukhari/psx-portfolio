...
export function validateInvestmentDossier(analysis, price, referenceYear = new Date().getUTCFullYear()) {
  ...
  // relationship to revenue's magnitude (real case: MEBL, Meezan Bank -
  // OCF of Rs 881bn against Total Income of Rs 285bn, ~3.1x, is normal for
  // a deposit-taking bank). The ocf > revenue*2 check exists to catch a
  // genuine PKR billion/thousand unit-conversion mistake, which only
  // holds as a sanity check for a non-financial company.
  const isFinancialInstitution = /bank|insurance|takaful|modaraba|leasing|exchange compan|investment compan|\bdfi\b/i.test(String(analysis.sector || ''));
