...
export function validateInvestmentDossier(analysis, price, referenceYear = new Date().getUTCFullYear()) {
  ...
  if (notes.some((note) => !/(annual report|page\s*\d|PSX|rating|Reuters|Dawn|Business Recorder)/i.test(String(note))))
    throw Error('Every score explanation must identify the evidence source used.');
}
