...
export function validateInvestmentDossier(analysis, price) {
  ...
  if (
    ...
    financials.some((row) => row.verified !== true || !row.source || !row.page || !row.basis || typeof row.eps !== 'number' || typeof row.profit !== 'number')
  ) throw Error('Five directly cited annual periods are required before an investment evaluation can be completed.');
